...
재앙으로 닥치는것을 보아 이러한 작은 성능개선 사항을 정리하고자 합니다.

이번 post에서는 검색엔진을 구현할때 API호출을 줄이는 방법으로서 지연 호출을
하는 방법에 대해서 알아보겠습니다.
...
setTimeout은 비동기적으로 설정한 delay 이후에 실행할 수 있도록 하는 함수이다.
...
# 구현
구현 방법은 싱글톤과 유사하다. setTimeout으로 delay 이후에 API 호출 하는데
